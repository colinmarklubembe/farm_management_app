...
<html>
<head>
    <title>Manage New Animals Bought</title>
...
<body>
    <h1>Manage New Animals Bought</h1>

    <form action="manage_new_animals.php" method="post">
        ...
        <input type="number" name="unit_cost" step="0.01" required min="1">

        <label for="tag">Tag:</label>
        <input type="text" name="tag" required>

        <input type="submit" value="Add New Animals Bought">
    </form>
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($conn) {
            ...
            // Insert the new animals into the "animals" table with their respective tags
            ...
            // Calculate the total cost and insert the record into the "new_animals_bought" table
            $totalCost = $quantity * $unitCost;
            $query = "INSERT INTO new_animals_bought (animal_type, quantity, unit_cost, total_cost) VALUES (?, ?, ?, ?)";
            $stmt = mysqli_prepare($conn, $query);
            mysqli_stmt_bind_param($stmt, 'sddd', $animalType, $quantity, $unitCost, $totalCost);

            if (mysqli_stmt_execute($stmt)) {
                echo "<p>New animals bought added successfully.</p>";
            } else {
                echo "<p>Error adding new animals bought.</p>";
            }
    ...
    <?php
    // Code to retrieve and display all available records from "new_animals_bought" table
    include 'connect.php';

    if ($conn) {
        $query = "SELECT * FROM new_animals_bought";
        $result = mysqli_query($conn, $query);

        if ($result && mysqli_num_rows($result) > 0) {
            echo "<h2>Available New Animals Bought</h2>";
            echo "<table>";
            echo "<tr><th>Animal Type</th><th>Quantity</th><th>Unit Cost</th><th>Total Cost</th></tr>";
            ...
            echo "<p>No new animals bought records found.</p>";
        }
